...
void main (void)
  {
      ...
      while(1)
      {
          ...
          distance1 = uart1[3] + (uart1[4] * 256);//lidar on c port 
          distance2 = uart2[3] + (uart2[4] * 256);//lidar on d port
          encoder1_read = encoder1_value();
          encoder2_read = encoder2_value();

          drive_equation(xj1,-yj1,correct_gyro);
      }
  }
...
void all_init()
{
      ...
      uart4rx_init();//lidar on c port
      usart5rx_init();//lidar on d port
